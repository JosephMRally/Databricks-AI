...
engineer | split each `|`-joined `emails` value into elements, split comma-separated addresses within an element, strip display names (`Alice <a@x.com>` -> `a@x.com`), and dedup | the extract carries raw header values 1:1; this phase owns the Transform of addresses so downstream matching against user-entered addresses works
engineer | every email address lower-cased before any processing (dedup, aggregation, output) | `Alice@X.com` and `alice@x.com` are the same sender, so a thread's `emails` never holds case duplicates and downstream phases can rely on lowercase
