...
engineer | one output row per dormant **address per deletable thread**, with dormancy aggregated across every thread the address appears in: first seen = min of the threads' `earliest_date`, last seen = max of their `latest_date`, each row carrying a single `thread_id` and that thread's `message_id`s | an address still active in any thread is not dormant, and the cleanup step gets one deletable thread per row with the message ids it needs
...
engineer | **ignore excludes at address level only**: an ignored address never appears in any output row, but threads containing it stay eligible | the owner is on essentially every thread — thread-level exclusion for ignore would empty the delete list
...
engineer | this script reads the input CSV and writes the delete-list CSV and its companion queries text file, nothing else — no Gmail access, no deletion | deletion is a separate, deliberate action, never a side effect of computing the list; the `thread_ids` and `message_ids` columns give that step everything it needs
...
engineer | the array column (`message_ids`) in the output CSV to be `|`-delimited | consistent with the rest of the pipeline
...
human | alongside the CSV, a text file of the delete-list addresses, one address per line (each address once, however many thread rows it has), defaulting to `filtered_queries.txt` (overridable via `--queries-out`) | a plain list is easy to copy-paste and easy for other tools to consume line by line
...
`thread_ids`, str; a single thread id — one output row per (address, thread)
`message_ids`, array<str>, `|`-joined; the message ids of this row's thread
`emails`, str, normalized (lowercase, bare address, as produced by the aggregate)
